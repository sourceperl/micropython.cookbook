...
I2C_SDA_PIN = const(0)
I2C_SCL_PIN = const(1)
# antenna tuning capcitance (must be integer multiple of 8, form 8 to 120 pf)
AS_CAP_PF = const(96)
...
i2c.writeto_mem(AS_I2C_ADDR, AS_REG_8, bytes([AS_CAP_PF//8]))
sleep_ms(2)
...
while True:
    ...
    as_int_status = i2c.readfrom_mem(AS_I2C_ADDR, AS_REG_3, 1)[0]&0x07
    # process status on need
    if as_int_status == AS_INT_NOISE_HIGH:
        log('noise level too high')
    elif as_int_status == AS_INT_DIST_DETECT:
        log('disturber detected')
    elif as_int_status == AS_INT_LIGHTNING:
        log('lightning occur')
        # get distance
        ...
        log(f'raw energy is {energy_raw}')
        # sound alert
        beep()
    # before next loop
    sleep_ms(10)
